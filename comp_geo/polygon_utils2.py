...
def get_normal_to_polygon_at_point_of_intersection(the_boundary,P_i,v_normal,dr,graff=False,verbosity=0):
    """
     the normal might point into the shape or outside

      1) we get the intersection point
      2) determine the normal from the line segment
      3) put a test point in both directions
      4) test if they are outside
      5) if both points are outside you are in a sharp corner and overshot
      6) in this case we look at the number of intersections between the test point and the interesection point
      7) if they are equal we divide the test point distance by 2 and call it recursively
    ...
    #dr=length_scale     
    P_test_1=point_utils.move_point_along_vector(P_i, v1, dr)
    P_test_2=point_utils.move_point_along_vector(P_i, v2, dr)

    outside_1=not the_boundary.contains(P_test_1)
    outside_2=not the_boundary.contains(P_test_2)
    
    if outside_1 and not outside_2:
        return v1

    if not outside_1 and outside_2:
        return v2

    ### both directions are outside so we are at sharp corner
    ### literally a corner case
    if outside_1 and outside_2:
        list_of_boundary_segments=shapely_utils.polygon_exterior_to_array_of_linestrings(the_boundary)
    
        #the_line=list_line.linestring_to_listline(the_linestring)
        
        L1=list_line.ListLine([P_i, P_test_1])
        
        L2=list_line.ListLine([P_i, P_test_2])

        object_list=list_of_boundary_segments
        poi_df_1 = L1.get_all_intersections_as_df(object_list)

        object_list=list_of_boundary_segments
        poi_df_2 = L2.get_all_intersections_as_df(object_list)

        if verbosity > 0:
            s=os.path.basename(__file__) + "."
            s+= inspect.currentframe().f_code.co_name
            print(f"In {s} ")
            print(f"len(poi_df_1)={len(poi_df_1)}")
            print(f"len(poi_df_2)={len(poi_df_2)}")
        ...
        if graff:
            ...
            #for tes_j in all_tes.values():
            #    plotting_utils2.plot_shapely_object(ax1, tes_j,'b')
    
            plotting_utils2.plot_shapely_object(ax1, L1,'b')
            plotting_utils2.plot_shapely_object(ax1, L2,'r')
            
            plotting_utils2.plot_shapely_object(ax1,the_boundary.boundary,color="green",show_points=False,linestyle='solid',linewidth=3)
            ax1.axis('equal')
            plt.draw()
            #plt.pause(0.1)
        ...
        return get_normal_to_polygon_at_point_of_intersection(the_boundary,P_i,v_normal,dr/2)
        #return v1
        assert(False)
